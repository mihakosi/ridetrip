...
const getRecurringRides = (req, res) => {
  Recurring.findAll({
    where: { userId: req.payload.id, offered: false },
  })
    .then((recurring) => {
...
const getRecurringRide = (req, res) => {
  Recurring.findOne({
    where: { id: req.params.id, userId: req.payload.id, offered: false },
  })
    .then((recurring) => {
      if (recurring) {
        return res.status(200).json(recurring);
...
const getRecurringOffers = (req, res) => {
  Recurring.findAll({
    where: { userId: req.payload.id, offered: true },
  })
    .then((recurring) => {
      return res.status(200).json(recurring);
    })
...
const getRecurringOffer = (req, res) => {
  Recurring.findOne({
    where: { id: req.params.id, userId: req.payload.id, offered: true },
  })
    .then((recurring) => {
      if (recurring) {
        return res.status(200).json(recurring);
