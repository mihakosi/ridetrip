...
const getVehicles = (req, res) => {
  Vehicle.findAll({ attributes: ["id", "model", "licencePlate", "passengers", "baggage"], where: { ownerId: req.payload.id } })
    .then((vehicles) => {
...
const getVehicle = (req, res) => {
  Vehicle.findOne({
    ...
    where: { id: req.params.id, ownerId: req.payload.id },
  })
    .then((vehicle) => {
      if (vehicle) {
...
const updateVehicle = (req, res) => {
  if (!req.body.model) {
    ...
    Vehicle.findOne({ where: { id: req.params.id, ownerId: req.payload.id } })
      .then((vehicle) => {
        if (vehicle) {
          vehicle
            .update({
